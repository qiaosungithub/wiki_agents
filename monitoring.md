...
| `blaze_reaper.sh` | Legacy `*/30` cron entry now logs delegation and exits without signaling. The independent `swap-oom-agent.service` monitor is the sole resource reaper and uses the same `/tmp/blaze-reaper.lock`. Reclaim requires exact process identity, completed-command evidence, verified idle time, and no active children/clients/output locks/queued use; process age or a missing profile alone never establishes idle. |
