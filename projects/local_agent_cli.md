...
wiring; the agent web app is `agent_web.md`. All of this runs on
`sqa-large.c.googlers.com`; the migration from `sqa`, the re-sync recipe, and
what still runs where are in `workstation.md`. The amply gateway URL is in
...
runbook for the gateway and its local database. Chapter 3 is the classic ways a
run only looks dead.
...
## Chapter 2 — Operating The Amply Service
...
from the version-skew 500 (`../engineering.md`, which 500s the *read/status*
...
its denominator (`../engineering.md` §Communicating a result). The util divides
for you; trust `/core`, not the first column.

---

## Chapter 3 — Classic Bugs: A Run That Only Looks Dead
...
behind it (`../engineering.md` §A tool call only fires as a structured call)
