...
|---|---|---|
| Type 1: Kaiming Group code | `jax_llava`, `PaliGemma-baseline`, `beifen-Paligemma`, `beifen` | Data, checkpoints, and compute stay in one region; read `data_locality.md` |
| Type 2: Google internal research code | `project_one_ssl`, `one-benchmark-suite`, `nnflow_jax`, `EqR`, `EqR-jax` | The Type 1 cross-region prohibition does not apply; read `data_locality.md` before choosing runtime storage |
...
- `project_one_ssl` has deliberate architectural constraints: the joint
  ...
  Verify any launch section against `xmanager.md` and the current wrapper.
