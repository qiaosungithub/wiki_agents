...
tiers, price caps, and the data behind them. Hub is `README.md`; the income/10
spend gates that sit on top of this market are `budget.md`, and the `tpu` tool
that reads it is `tpu_cli.md`. Read this when a job will not schedule and
`../jobs.md` does not explain it, or before setting any price cap.
...
(`../jobs/submit.md` Chapter 1 §Groups). These mechanisms change: live state and
...
**Set one with `../tools/limit_order.sh`; it is read-first and refuses group
...
name in others (`../tpu_reference.md` has the mapping).
...
## Errors: Why A Job Will Not Schedule
