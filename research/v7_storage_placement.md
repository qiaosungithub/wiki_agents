...
`dfw` was added to unlock cheap v4). Two more carry PARTIAL mirrors and must not
be treated as full: `las` (the maze v4 working set) and `sin` (the four
GPU/B200-line corpora). Each was chosen on live obtainability, storage headroom,
and a verified smoke, weighting *breadth of cells* because a one-cell metro is
one stockout away from useless.
...
**`las`/`dl-d` and `sin`/`si-d` are partial mirrors — do not assume a dataset is
there.** `las` holds maze64's v4 working set; `sin` holds only the four
GPU-line corpora above (in particular it has ARC-1 **and** ARC-2, where `las`
and `dfw` carry ARC-1 only). `storage.md` §Existence Is Not Completeness applies:
check for the dataset's `_MIRRORED`/`_SUCCESS` on the cell before pinning a job
...
**`sin` is the B200 metro, and `sj` — the only cell with B200s in quantity —
has ZERO storage registrations of its own**, so a B200 job necessarily reads a
sibling cell and the mirror map must name it; unlisted, it falls through to the
far default, which is the pruner-kill case. `si-d` is that sibling: of the nine
registered `sin` storage cells it has both the most free space and an sp50
spindle commitment, 3.3x the free space of the runner-up. **Do not pick a
storage cell by campus adjacency to the compute** — `sj` sits on campus `lyw`
and `si-d` on `wen`, but metro is the boundary that matters and same-metro
cross-campus reads are free, so the campus-mate (`sh-d`, sp10) would trade a 5x
lower throughput floor for nothing measurable.
...
| ~~`sin`~~ | **No longer rejected — now a PARTIAL data metro** (`si-d`), stood up because it is the only metro with B200 compute. Its storage really is the thinnest of any candidate (9.25 PiB), which is why it carries the GPU-line corpora rather than a full mirror. See the standing-decision table above. |
