...
probes, plus a v5p-128 probe and two observed v6e-64 jobs. Every hold is keyed on
the Borg `started` epoch. Raw data: `$AMPLY_ARTIFACT_DIR` of run
`20260810-151959-5eb6c14e` (`episodes.tsv`, `REPORT.md`, `FINDINGS.md`).
...
How wrong a remembered ranking gets: the 18.7 h window below (2026-08-10) found
v6e the most reliable and v6p the worst. Ten days later (2026-08-21) v6e and v5e
PROD were **limit-order-blocked pool-wide** (un-gettable at any cell), v5p
cleared at **0.0**, and v7 cleared **cheaper than v6p** — the exact inversion.
The window is kept below only as *method evidence* for the durable rule (**a
slice you cannot hold has no throughput**), never as a card recommendation. The
...
hold, most episodes cannot save before being preempted: one 2-hour stretch of 12
acquisitions produced **zero** completed checkpoints. Warm throughput was a
healthy 16 steps/min; net throughput was ~5% of a v7-32 running the same code.
...
**A capacity table does not predict acquisition.** Aligned against the real queue
minute by minute, **14 of 16 live-price samples said `capped` while the queue
was holding or granting v6p-64** — 12.5% accurate. The cause is a granularity
mismatch: preflight reads a *group*-level window while grants are *cell*-level
and opportunistic. Details and the price-cache trap in
...
(exact permutation test, p=0.0187). Within the good era one cell was better (tul
median 23.9 vs 2.5 min, p=0.0130) — but in the bad era *that same cell was the
worst*. So "just use tul" is wrong; a short probe now beats any remembered
ranking.

**Beware confounds when comparing cells.** The cell effect above was half an
artifact: tul was also sampled earliest and most, so "cell" and "hour of night"
were confounded. Before believing any grouping, ask whether the groups differ in
*when* they were sampled. Deconfound by launching the same ask into a second
cell/group in the same window.
