...
or an infra failure is not (`result_logging.md`).

| Read | When |
|---|---|
| `looped_nanogpt_per_site.md` | The core research idea page: the per-site optimizer hypothesis (call-site normalization, distance weighting, update-scale alignment, relative-distance grouping) and the clean looped-nanoGPT setting (parcae / loopformer / ouro) that tests it. |
...
spreadsheet with its chart link (`result_logging.md`).

Identify the actual tracking backend before querying it. `EqR-jax` routes
WandB-shaped calls to DeepMind Datatables (`../projects/eqr_jax.md`), and
